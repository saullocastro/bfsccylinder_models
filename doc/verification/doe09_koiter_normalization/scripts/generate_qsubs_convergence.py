...
--eps1 0.0005, on the edges of the models, SS3 with inertia relief
(bfsccylinder_models/edges.py): v = w = 0 along both edges, the axial load
on both edges, no node anchored.

Every case at ny = 24, 32, 40, 48, 64, 80, 96, 120 and 160 and at the axial
factors F = 0.5, 1 and 2, the largest axial element length being dy/F:
elements twice as long axially as around, square, and half as long; then
ny = 200 and 240 at F = 2, and F = 3 at ny = 120 and 160; 93 runs. The
same study was run with free edges too (free-IR, an option since removed
from the models), see REPORT.md.
...
def tag(r):
    return ('SS3IR_F%g_eps0p0005' % r['axial_factor']).replace('.', 'p')
...
for icase in cases:
    for F in axial_factors:
        for ny in nys:
            runs.append(dict(icase=icase, ny=ny, axial_factor=F))
#NOTE the extension, after the first runs, whose b at ny = 160 (5.3
#     elements per wave of n_c = 30) was still 2 to 12 % from its Richardson
#     estimate: ny = 200 and 240 at F = 2, and F = 3 at ny = 120 and 160 for
#     the axial mesh
for icase in cases:
    for ny in [200, 240]:
        runs.append(dict(icase=icase, ny=ny, axial_factor=2.))
    for ny in [120, 160]:
        runs.append(dict(icase=icase, ny=ny, axial_factor=3.))
for r in runs:
