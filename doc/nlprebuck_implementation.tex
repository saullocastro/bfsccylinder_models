...
Except for $\phi_2$, which is \texttt{KC + KG*mu[0]}, they are integrated by
\texttt{koiter\_element\_tensors} of \texttt{koiter\_tensors.py}, the one
element loop of the four models, which are left to supply only
$\boldsymbol{G}_x$, $\boldsymbol{G}_y$ (von Karman or Sanders) and the
membrane and coupling stiffness at the integration points
(Section~\ref{sec:element-cost}).

\subsection{Cost of the element loop}
\label{sec:element-cost}

Up to version 0.3.2 the element loop was repeated in each of the four models
and evaluated every tensor with Python loops over the modes at every
integration point: over the $n_c$ directions of the null space for
$\phi_{20}$, over the $m^2$ pairs of Koiter modes for $\phi_3$, $\phi_{30}$,
$\phi_{200}$ and the constants of the orthogonality conditions, several
\texttt{np.einsum} calls with path optimisation per pair, and over the $m^4$
quadruples for $\phi_4$, twelve small matrix products each.  With $m = 5$ that
is about $10^5$ NumPy calls per element and $0.7$ to $2$~s per element on one
core, depending on the machine, which for $m = 5$ at the production mesh of
the design of experiments of Section~\ref{sec:cts} made the Koiter tensors cost
more than five times the rest of the analysis.

\texttt{koiter\_element\_tensors} evaluates the same terms with the modes and
the $n_{int}^2 = 16$ integration points of an element as array axes.  Every
term of the old loops becomes one contraction over the integration points
and the three strain components, a single matrix product per element whatever
$m$.  Three things make that possible without changing the arithmetic beyond
round-off:
\begin{itemize}\itemsep2pt
  \item $\partial^2\boldsymbol{\varepsilon}/\partial\boldsymbol{\bar{u}}^2$ is
        never formed.  Applied to a mode it is $\boldsymbol{G}_x^\top$ and
        $\boldsymbol{G}_y^\top$ scaled by $\boldsymbol{G}_x\boldsymbol{u}$ and
        $\boldsymbol{G}_y\boldsymbol{u}$, so its contractions with all the
        directions at once are outer products.
  \item The terms of $\phi_3$, $\phi_{30}$ and $\phi_{200}$ come in pairs that
        are the same contraction with the two mode indices swapped, so each
        pair is computed once and its second member is a transpose.
  \item $\phi_4$ is accumulated as
        $P_{abcd} = \sum w\, (\boldsymbol{A}\boldsymbol{E}_{ab})\cdot
        \boldsymbol{E}_{cd}$, with $\boldsymbol{E}_{ab}$ the second-order
        strain of modes $a$ and $b$.  The six terms of the old loop are six
        permutations of it, which are applied once after the loop over the
        elements.  With $\boldsymbol{A}$ symmetric they are three equal pairs;
        all six are kept, which costs nothing and does not rely on it.
\end{itemize}
\texttt{tests/test\_koiter\_tensors.py} compares the result against the old
loop, kept in the test as it was in version 0.3.2, on random elements with
both kinematics and with and without \texttt{NLprebuck}: every tensor agrees
to $5\times10^{-16}$ of its largest entry.

The right-hand side of the bordered system, Eq.~\ref{eq:bordered}, is
symmetric in $i$ and $j$, so $\boldsymbol{\bar{u}}_{ij} =
\boldsymbol{\bar{u}}_{ji}$ and only the $m(m+1)/2$ fields with $i \le j$ are
solved for, instead of $m^2$.  Version 0.3.2 solved for both.  On DOE case 6
at $n_y = 40$ its $\boldsymbol{\bar{u}}_{ij}$ and $\boldsymbol{\bar{u}}_{ji}$
differ by at most $5\times10^{-13}$ of the largest
$|\boldsymbol{\bar{u}}_{ij}|$, and the fields of the two versions by
$7\times10^{-13}$.  On the Waters shell, solved with SuperLU, the two figures
are $2\times10^{-8}$ and $1.5\times10^{-7}$: the bordered system amplifies
...
(\texttt{bordered\_roundoff\_floor.py}).  That is the floor any change to the
order of summation meets on this shell, the vectorization included.

On one core, the element loop costs $0.43$, $0.49$, $0.66$ and $1.06$~ms per
element for $m = 1$, $2$, $5$ and $8$, against $14$, $52$, $1100$ and
$5800$~ms for the loop of version 0.3.2 on the same elements, $34$ to $5500$
times less (\texttt{element\_loop\_timing.py}).  It is now nearly independent
of $m$, where it grew as about $m^4$.  What remains of the Koiter cost is the
bordered solves: $m(m+1)/2$ of them, whose cost grows faster than the number
of elements.  On DOE case 6 at $n_y = 40$ and the Waters shell at
$n_y = 60$, 960 elements, the Koiter section is $14$ to $38$ times faster for
$m = 2$ and $5$.  Pcr and the multipliers are identical to the last bit, the
eigenvalue analysis being unchanged; $b_{ijkl}$ agrees to $10^{-13}$ of its
largest entry on the DOE case and to the floor above, $9\times10^{-11}$, on
the Waters shell.  The non-zero $a_{ijk}$, which occur only for an
axisymmetric mode, agree to $10^{-14}$, and peak memory is unchanged.  At the
production mesh of the design of experiments, $n_y = 160$, DOE case 0 with
\texttt{NLprebuck=True} and $m = 5$ on a core of the cluster, the Koiter
section takes $16.8$~ms per element, of which the element loop is $1.3$ and the
bordered solves $15.3$.  Over 12,800 to 25,600 elements the time per element
grows by $25$ per cent, following the bordered solves.  The old loop runs at
$2.0$~s per element on those cores, so the Koiter section of that run is about
$120$ times, and the whole run about $30$ times, faster; this is estimated
from the element loop alone, the baseline run not having been completed there
(\texttt{doc/verification/cluster\_results/REPORT.md}).
...
\texttt{element\_loop\_timing.py}
  & Section~\ref{sec:element-cost} & $\sim$1~min \\
\texttt{benchmark\_koiter\_vectorization.py}
  & Section~\ref{sec:element-cost} & hours \\
\texttt{bordered\_roundoff\_floor.py}
  & Section~\ref{sec:element-cost} & $\sim$2~min \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{test\_koiter\_tensors.py}
  & random elements
  & the element loop of version 0.3.2, Section~\ref{sec:element-cost} \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The last of these compares the CTS and the constant-stiffness modules
...
comparison.  The element loop of the Koiter tensors needs no such test, the
four models calling the same \texttt{koiter\_tensors.py};
\texttt{test\_koiter\_tensors.py} checks it instead against the Python loops
it replaced, kept in the test as they were in version 0.3.2.
