...
Two pieces of the implementation used to stand between this and a multi-mode
analysis.  The constant of the orthogonality condition was taken from the
...
directions than that.  Both are now generalised
...
The partner has to be rebuilt more often than not.  On the $n_y = 40$ meshes
of both reference shells, with the two and four multipliers the tests ask
for, the eigen solver returns no partner at all: the Arbocz shell gives
$n = 10$ and then $n = 11$, $0.19\%$ above, and the Sun shell
$n = 10, 9, 7, 11$.  The column border used to hold the critical mode alone.
...
checks that the rebuilt partner lies in the column border on a mesh where the
eigen solver does not return it.
...
\begin{enumerate}\itemsep3pt
  \item \textbf{Degenerate critical modes.}  The rotation inside the
        ...
        $n_y = 40$ mesh.  The canonicalisation of
        ...
        null space (Section~\ref{sec:secondorder}).
...
\texttt{reference\_b\_convergence.py}
  & Sections~\ref{sec:canonical} and \ref{sec:gap} & 2--25~min per run \\
\bottomrule
\end{tabular}
\end{center}
