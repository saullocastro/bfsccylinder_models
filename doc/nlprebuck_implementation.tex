...
is about $10^5$ NumPy calls per element and @@BASE_MS_PER_EL@@~ms per element
on one core, which for $m = 5$ at the production mesh of the design of
experiments of Section~\ref{sec:cts} made the Koiter tensors cost five times
...
solved for, instead of $m^2$.  Version 0.3.2 solved for both, and its
$\boldsymbol{\bar{u}}_{ij}$ and $\boldsymbol{\bar{u}}_{ji}$ differ by at most
@@UIJ_ASYM@@ of the largest $|\boldsymbol{\bar{u}}_{ij}|$, which is round-off;
the fields of the two versions differ by at most @@UIJ_DIFF@@.

Measured with \texttt{doc/verification/benchmark\_koiter\_vectorization.py}
on the DOE cases 0, 1 and 6 at $n_y = 40$ and $80$ and on the Waters shell,
one core each, the element loop now costs @@NEW_LOOP_MS@@~ms per element for
$m = 5$, and the whole Koiter section, $t(m) - t(0)$, is @@SPEEDUP_RANGE@@
times faster than in version 0.3.2 for $m = 5$
(\texttt{doc/verification/README.md} has the tables).  Pcr, the multipliers,
$a_{ijk}$ and $b_{ijkl}$ agree with version 0.3.2 to @@MAX_REL_B@@ of the
largest entry of each, and peak memory is unchanged.  What remains of the
Koiter cost is the bordered solves, whose number grows as $m^2/2$ and whose
cost grows faster than the number of elements; the element loop grows
linearly with the number of elements and about as @@M_SCALING@@ with $m$.  At
the production mesh of the design of experiments, $n_y = 160$, case 0 with
\texttt{NLprebuck=True} and $m = 5$, the Koiter section takes
@@KOITER_160_MS@@~ms per element, of which the element loop is
@@LOOP_160_MS@@.
